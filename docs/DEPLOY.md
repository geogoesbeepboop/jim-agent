...
be _understood_, not just executed. Every step you run yourself; jim never
...
| `ANTHROPIC_API_KEY`                     | your key                 | synthesis/judge; gather + gate run without it. **Production uses API-key auth only** — the seller/monitors pin it (ADR-0010)       |
| `LLM_AUTH_MODE`                         | `api_key`                | leave as `api_key` in production; `subscription`/`auto` are dev-loop only (evals, local `jim-research`) and never back paid output |
...
- **Watch:** `/proof` (public), `/admin` (settlements), `/dashboard` (margin +
  ...
  announcements.
- **Backups:** Supabase/Railway PG backups on; the store _is_ the audit trail.
